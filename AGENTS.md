...
- UI：设置页「检查更新」+ 启动（打包后）延迟 3s 静默检查；发现新版本自动下载，标题栏右上角（最小化左侧）上箭头更新按钮（可手动检查；下载中变黄显示进度角标点击可暂停、暂停中点击继续，就绪变品牌色点击安装并重启）+ 品牌区后状态中心显示更新条目（检查中 / 下载中进度 / 下载已暂停可点击恢复 / 发现新版本与就绪可点击操作），设置页展示进度、进度行内提供「暂停/继续」「取消」（取消检查回到 idle、取消下载回到可重下）与「安装并重启」；更新下载复用 `downloadEngine`（独立实例，不混入用户下载列表），暂停/恢复走引擎断点续传，取消通过 `checkSeq` 使在途检查失效 + 取消引擎任务。
