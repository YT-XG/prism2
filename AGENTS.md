# Prism v2（prism-next）— 项目主文档

> 从旧仓库（父目录的 electron-vite-learn）全新搭建的第二代 Prism。
> 代码规范见 [docs/conventions.md](docs/conventions.md)，设计系统见 [docs/design.md](docs/design.md)。
...
4. 改 IPC / 数据模型 → 同步 `@preload/ipc.ts` + `docs/services.md`；新增文件 → 更新本文目录索引。
5. `npm run typecheck` 零错误；不提交 `out/`、`dist/`、`node_modules/`。

## 与旧仓库的隔离

- 独立 git 仓库（父仓库已 ignore `prism-next/`）。
- `package.json` name = `prism-next`，userData 与旧版（`Prism`）不同，互不干扰数据/全局快捷键/托盘。
...
| 翻译、Markdown 预览、下载、文件互传、弹窗族、OCR | ⬜ 见 docs/migration-roadmap.md |
