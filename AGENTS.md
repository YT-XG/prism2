...
- **自托管更新服务器镜像**（`filedev.wxgx.com/prism2`，2026-09 落地；主通道已改为「CI 直推」）：GitHub Actions 发版后把 win/mac 安装包 + 服务器版 `latest.json` 经 **443** 上传到服务器 `/prism2-upload/`（PUT + 请求头 `X-Prism2-Token`，token = `PRISM2_WEBHOOK_TOKEN`），再经 webhook 触发服务器 `prism2-sync.sh` 的「本地新鲜短路」秒级 finalize（补写 marker + 清理旧版本，零网络）；代理下载（ghfast.top/ghproxy.net）仅作**推送失败时的兜底**。客户端经 Gitee `redirect` 跳转到服务器下载，GitHub 保持自包含兜底。详见 `../docs/prism2/self-host-update.md`。
