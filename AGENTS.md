...
- **自托管更新源（阿里云 OSS，替代低带宽 ECS）**：客户端经 Gitee 锚点 `redirect` 跳转到 **阿里云 OSS** 下载，GitHub Release 保持自包含作兜底。GitHub Actions 发版后用 oss2 SDK 把 win/mac 安装包 + OSS 版 `latest.json` 上传到 OSS bucket（`OSS_BUCKET`/`OSS_REGION`/`OSS_KEY_PREFIX`，AccessKey = `OSS_ACCESS_KEY_ID`/`OSS_ACCESS_KEY_SECRET`）；`PRISM2_UPDATE_BASE`（缺省 `https://prism2.oss-cn-hangzhou.aliyuncs.com/prism2`）指定 `binaries[].url` 前缀。OSS 未配置时自动回退 GitHub 兜底（Gitee 锚点仍可 404→GitHub）。详见 `../docs/prism2/self-host-update.md`。
