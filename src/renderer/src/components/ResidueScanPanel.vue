      ...
      <template v-if="result && !scanning">
        ...
        <template v-else>
          <div v-for="group in groups" :key="group.title" class="rp-group">
            <button
              type="button"
              class="rp-group__title"
              :aria-expanded="!collapsedGroups.has(group.title)"
              @click="toggleGroup(group.title)"
            >
              <span class="rp-group__title-main">
                <ChevronDown
                  :size="15"
                  :stroke-width="1.7"
                  class="rp-group__chevron"
...
import { ScanSearch, ChevronDown } from '@lucide/vue'
...
.rp-group__title:hover {
  color: var(--text-primary);
}

.rp-group__title:focus-visible {
  outline: none;
  box-shadow: var(--ring);
  border-radius: var(--radius-sm);
}

.rp-group__title-main {
  display: inline-flex;
  align-items: center;
  gap: var(--sp-2);
}
...
.rp-group__chevron.is-collapsed {
