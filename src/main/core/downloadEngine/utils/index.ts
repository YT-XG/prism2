...
export function buildChunks(totalBytes: number, threads: number, tempDir: string): ChunkInfo[] {
  const targetThreads = Math.max(
    1,
    Math.min(threads, Math.floor(totalBytes / MIN_CHUNK_BYTES) || 1)
  )
  ...
  for (let index = 0; index < targetThreads; index++) {
    const end =
      index === targetThreads - 1 ? totalBytes - 1 : Math.min(totalBytes - 1, start + chunkSize - 1)
    chunks.push({
      index,
      start,
      end,
